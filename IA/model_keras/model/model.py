...
def make_model(input_shape, num_classes, last_activation="linear", nb_modules=4, reduction_layer="globalavgpool",
    ...
    for size in liste_filtres[:-1]:
        ...
        x = SeparableConv2D(filters=size, kernel_size=3, padding="same",
                        kernel_regularizer=regularizers.l1_l2(l1=1e-5, l2=1e-4),
                        bias_regularizer=regularizers.l2(1e-4))(x)
        x = BatchNormalization()(x)
        ...
        # Project residual
        residual = Conv2D(filters=size, kernel_size=1, strides=2, padding="same",
                        kernel_regularizer=regularizers.l1_l2(l1=1e-5, l2=1e-4),
                        bias_regularizer=regularizers.l2(1e-4),
                        activity_regularizer=regularizers.l2(1e-5))(previous_block_activation)
        x = Add()([x, residual])  # Add back residual
        previous_block_activation = x  # Set aside next residual

    x = SeparableConv2D(filters=liste_filtres[-1], kernel_size=3, padding="same",
                        kernel_regularizer=regularizers.l1_l2(l1=1e-5, l2=1e-4),
                        bias_regularizer=regularizers.l2(1e-4))(x)
    x = BatchNormalization()(x)
    x = Activation(activation="relu")(x)
