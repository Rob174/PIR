...
class MakeConfusionMatrix:
    ...
    def add_sample(self,batch_img, batch_true):
        ...
        batch_true = np.array(self.fonction_approx(batch_true[:, :].numpy()), dtype=np.int)
        batch_pred = np.array(self.fonction_approx(batch_pred[:, :]),dtype=np.int)
    ...
    def save_confusion_matrix(self,matrice_confusion_petite,labels,classe_index,identifieur_nom=""):
        ...
        plt.title(
            f"Confusion matrix of class {classe_index} {identifieur_nom}: {self.correspondances_index_classes[classe_index]} \nPrecision : {matrice_confusion_prct[-1, -1]:.2f} % ; Top-3 precision : {top3voisins_precision:.2f} %")
        ax = plt.gca()
