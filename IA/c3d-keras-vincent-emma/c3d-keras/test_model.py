...
def main():
    ...
    vid = []
    with open ("../../bdd/nuscenes_extraction/extracted_scene_annotation.json") as image:
        input = json.loads(image.read())
        for image in input[1]['images']:
            vid.append(cv2.resize(cv2.imread("../../bdd/nuscenes_extraction/data/"+image),(171,128)));
